...
export function applyToggleHotkeys(hotkeys: HotkeySettings): void {
  ...
  if (hotkeys.micMuteHotkey) {
    ...
    if (registered) registeredMicMuteAccelerator = hotkeys.micMuteHotkey.accelerator;
    else appendLog({ category: LogCategory.HOTKEYS, level: LogLevel.WARNING, message: HOTKEYS_STRINGS.acceleratorConflictMessage(hotkeys.micMuteHotkey.label) });
  }
  ...
  if (hotkeys.deafenHotkey) {
    ...
    if (registered) registeredDeafenAccelerator = hotkeys.deafenHotkey.accelerator;
    else appendLog({ category: LogCategory.HOTKEYS, level: LogLevel.WARNING, message: HOTKEYS_STRINGS.acceleratorConflictMessage(hotkeys.deafenHotkey.label) });
  }
}
