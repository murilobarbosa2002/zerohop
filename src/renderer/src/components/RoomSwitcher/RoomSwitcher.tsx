          ...
          <div key={session.sessionId} className="relative group">
            <button
              ...
              className={`w-10 h-10 border flex items-center justify-center text-body-sm font-bold ${
                isFocused ? 'bg-accent text-white' : 'bg-panel-2 text-text hover:border-accent'
              }`}
            >
              {label}
            </button>
            {session.unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-danger text-white text-[10px] font-bold flex items-center justify-center border border-bg">
            ...
            <button
              ...
              className="absolute -bottom-1 -right-1 w-4 h-4 bg-danger text-white text-[10px] font-bold items-center justify-center border border-bg hidden group-hover:flex"
            >
              ×
            </button>
          </div>
