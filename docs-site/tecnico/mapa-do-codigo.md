...
`useRoomSessions.ts` ganhou um `useEffect` de bootstrap (guardado por um `ref` pra rodar só uma vez, mesmo sob `<StrictMode>`) que, ao montar, chama `createSession()` + `RoomClient.createRoom(...)` automaticamente pra duas salas, cada uma só se já tiver senha configurada **E o toggle de habilitar estiver ligado**:

- **Sala pessoal**: `desiredCode = getPersonalId()`, senha de `personalRoomPreference.ts`, exige `getPersonalAutoOpenEnabled()` (v0.36.28+, **ligado por padrão** — checkbox no `PersonalRoomCard.tsx`).
- **Sala automática** (nova peça, `services/autoRoomPreference.ts` — espelha `personalRoomPreference.ts` com chaves de `localStorage` próprias): `desiredCode = getAutoRoomId()`, senha própria, **mais uma lista de IDs de contato pra convidar automaticamente** (`getAutoInviteContactIds()`/`setAutoInviteContactIds()`), exige `getAutoRoomEnabled()` (v0.36.28+, **desligado por padrão** — checkbox no topo do `AutoRoomCard.tsx`, decisão explícita do usuário: sala pessoal é opt-out, sala automática é opt-in).

Essas sessões entram na lista normal de `sessions` (aparecem no `RoomSwitcher` assim que conectam), mas **não roubam o foco** — a tela de escolha (`pendingSession`) continua aparecendo normalmente pro usuário. Como agora dá pra ter três tipos de sala misturados no mesmo seletor (pessoal, automática, normal) sem nenhuma distinção visual, `RoomSwitcher.tsx` (v0.36.27+) e `RoomHeader.tsx` (v0.36.28+, mesma etiqueta só que DENTRO da sala, perto do código/senha) usam `services/roomKind.ts` (`getRoomKind(roomCode)`, extraído do que antes era lógica duplicada) pra comparar `roomCode` contra `getPersonalId()`/`getAutoRoomId()` e mostrar um selo (casinha pra pessoal, raio pra automática) — comparação simples porque o código da sala pessoal/automática é sempre esse ID fixo, então basta checar igualdade de string.
...
- **Tela**: `components/NotificationsScreen/` espelha `LogsScreen/` (lista + paginação + confirmação de limpar, mais exclusão individual por item e dois `<select multiple>` de filtro — tipo e categoria — v0.36.28+). Acesso por um ícone de sino novo na `TitleBar`, com uma bolinha de não-lida contada via `useNotifications()`.
...
- **Filtros dinâmicos em Logs e Notificações (v0.36.29+)**: as duas telas calculam as opções de cada `<select multiple>` via `useMemo` em cima dos `entries` que já existem (`[...new Set(entries.map(e => e.category))]`, mesma ideia pra `level`/`kind`) em vez de listar `Object.values(Enum)` fixo — uma categoria/tipo sem nenhum registro simplesmente não aparece na lista de opções do filtro. `LogsScreen.tsx` ganhou um segundo filtro (`LogLevel`, que já existia no dado mas nunca era filtrável) — `CategoryFilter.tsx` (pílulas de seleção única) foi removido, substituído por dois `<select multiple>` no mesmo padrão que `NotificationsScreen.tsx` já usava; a leitura das opções marcadas de um `<select multiple>` foi extraída pra `lib/selectValues.ts` (`readSelectedValues(event)`) pra não duplicar entre as duas telas.
