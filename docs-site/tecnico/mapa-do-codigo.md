...
`main/globalHotkeys.ts` usa `uiohook-napi` (dependência nativa, captura de teclado em nível de sistema) porque a API padrão do Electron (`globalShortcut`) só avisa tecla pressionada, nunca solta — insuficiente pra push-to-talk (que precisa saber quando soltar pra cortar o mic). O fluxo de "gravar uma tecla" na tela de Configurações pede ao processo principal pra escutar o **próximo** `keydown` global e devolver o keycode + um nome legível — assim o app nunca precisa converter entre o sistema de teclas do DOM (`KeyboardEvent.code`) e o do `uiohook-napi`, que são diferentes. Push-to-talk reaproveita o mecanismo de mute normal (`RoomClient.setMicMuted()`) em vez de um estado paralelo, pra manter o indicador visual e o som de mutar sempre corretos.

Como toda dependência nativa deste projeto, a captura de teclado carrega com `import()` dinâmico dentro de `startGlobalHotkeys()`, com `try/catch` — se falhar (sistema sem suporte, permissão negada), o recurso fica indisponível em vez de derrubar o app. O resultado (sucesso ou falha) é registrado na categoria "Teclas de atalho" da tela de Logs.
