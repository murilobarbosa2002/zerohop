...
| `globalHotkeys.ts` | Captura de teclado em nível de sistema via `node-global-key-listener` (dependência nativa) — push-to-talk funcionando mesmo com outro programa em foco.                                                            |
...
- **Atalhos de teclado, flags, auto-update** → `settings.json` (via `main/settings.ts`), porque só o main roda a captura global de teclado (`node-global-key-listener`) e decide se ativa flags do Chromium.
...
- **Push-to-talk** (`main/globalHotkeys.ts`): precisa saber quando a tecla é solta, pra cortar o mic — a API `globalShortcut` do Electron não avisa isso, só uma lib nativa de captura de teclado em nível de sistema dá eventos DOWN/UP de verdade. A gravação da tecla de PTT também passa pelo processo principal: pede pra escutar o **próximo** evento de tecla e devolve o nome dela — precisa ser assim (diferente do fluxo acima) porque o valor salvo tem que bater com o mesmo formato usado em tempo real, e o `KeyboardEvent.code` do DOM não é compatível com o nome de tecla dessa lib. Usa `node-global-key-listener` (v0.36.21+) — trocada de `uiohook-napi` depois de duas falhas de campo diferentes num mesmo computador (a lib às vezes nem iniciava, outras vezes iniciava mas não entregava nenhum evento de tecla durante a gravação). A troca é mais um experimento que uma correção garantida: essa lib funciona de um jeito bem diferente no Windows — sobe um executável separado (`bin/WinKeyServer.exe`, não assinado) e conversa com o processo principal por stdio, o que pode ser sinalizado por antivírus do mesmo jeito (ou até mais) que uma dependência nativa in-process. A vantagem real é que ela expõe `onError`/`onInfo` de verdade (`IWindowsConfig`), então agora um erro real do sistema chega até a tela de Logs em vez de falhar em silêncio — é a primeira vez que existe diagnóstico de verdade nesse ponto.
...
Como toda dependência nativa deste projeto, a captura de teclado carrega com `import()` dinâmico dentro de `startGlobalHotkeys()`, com `try/catch` — se falhar (sistema sem suporte, permissão negada, processo bloqueado por antivírus), push-to-talk fica indisponível, mas mutar mic/áudio por atalho continuam funcionando normalmente (não dependem dela, usam `globalShortcut` do Electron). O resultado (sucesso, falha ao carregar, timeout ao iniciar, ou erro em tempo real via `onError`) é registrado na categoria "Teclas de atalho" da tela de Logs — é o primeiro lugar a checar se um atalho parar de responder.
