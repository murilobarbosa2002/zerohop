...
**Resumo em uma frase:** vídeo, áudio, chat e as mensagens internas da sala são criptografados ponta a ponta e nunca ficam salvos em lugar nenhum — nem em disco, nem em servidor, nem acessíveis a quem fez este programa.
...
Vídeo, áudio e as mensagens que os participantes trocam entre si (lista de membros, status de compartilhamento, chat) trafegam usando **WebRTC**, que já criptografa tudo nativamente com **DTLS-SRTP**. Isso não é uma funcionalidade que o ScreenShare precisou construir — é uma garantia do próprio protocolo, presente em qualquer aplicação WebRTC (inclusive chamadas de vídeo de navegador).
...
Para dois computadores conseguirem se conectar diretamente pela internet, alguém precisa apresentá-los um ao outro primeiro — trocar os dados técnicos necessários (endereços, capacidades de rede). O ScreenShare usa o **broker público e gratuito do [PeerJS](https://peerjs.com/)** (`0.peerjs.com`) só para essa etapa inicial de apresentação.

Esse broker é um serviço de terceiro, fora do controle deste projeto. Ele enxerga metadados de conexão — por exemplo, que um participante com tal código de sala está tentando se conectar com outro — mas **nunca** o conteúdo de vídeo, áudio, chat ou qualquer mensagem da sala, porque essas nunca passam por ele: elas só existem na conexão direta entre os PCs, depois que a apresentação inicial termina.
...
O ScreenShare não coleta métricas de uso, não envia analytics, não registra quem usa o programa, quando ou como. Não existe nenhum mecanismo de coleta de dados embutido. Todo o código está neste repositório para qualquer pessoa confirmar isso por conta própria.
